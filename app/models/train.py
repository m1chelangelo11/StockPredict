...
class trainModel:
    ...
    def load_data(self,ticker):
        ...
        y_train = torch.from_numpy(np.load(os.path.join(path, f'y_train_{ticker}.npy'))).to(self.device)
        y_val = torch.from_numpy(np.load(os.path.join(path, f'y_val_{ticker}.npy'))).to(self.device)
        y_test = torch.from_numpy(np.load(os.path.join(path, f'y_test_{ticker}.npy'))).to(self.device)
        ...
        X_train = torch.from_numpy(np.load(os.path.join(path, f'X_train_{ticker}.npy'))).to(self.device)
        X_val = torch.from_numpy(np.load(os.path.join(path, f'X_val_{ticker}.npy'))).to(self.device)
        X_test = torch.from_numpy(np.load(os.path.join(path, f'X_test_{ticker}.npy'))).to(self.device)
        ...
        for epoch in range(self.num_epochs):
            ...
            if X_val is not None and y_val is not None:
                ...
                with torch.no_grad():
                    ...
                    self.val_predictions.append(val_pred.detach().numpy())
